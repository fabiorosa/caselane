...
function Submit({ label, quiet = false }: { label: string; quiet?: boolean }) { const { pending } = useFormStatus(); return <button aria-busy={pending || undefined} className={quiet ? "quiet pending-action" : "pending-action"} disabled={pending} type="submit"><PendingActionContent label={label} pending={pending} pendingLabel="Saving..." /></button>; }
