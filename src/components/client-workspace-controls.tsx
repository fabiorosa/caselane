...
export function ArchiveClientButton({ action }: { action: () => Promise<void> }) { return <form action={action}><button className="client-archive-action" onClick={(event) => { if (!window.confirm("Archive this client? Existing case history will remain available.")) event.preventDefault(); }} type="submit">Archive client</button></form>; }
