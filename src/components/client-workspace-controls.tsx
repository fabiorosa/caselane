...
function SaveContact({ label }: { label: string }) { const { pending } = useFormStatus(); return <button className="contact-save" disabled={pending} type="submit">{pending ? "Saving…" : label}</button>; }
function ArchiveContact({ action }: { action: () => Promise<void> }) { const { pending } = useFormStatus(); return <button aria-live="polite" className="danger-text" disabled={pending} formAction={action} onClick={(event) => { if (!window.confirm("Archive this contact?")) event.preventDefault(); }} type="submit">{pending ? "Archiving…" : "Archive"}</button>; }

export function ArchiveClientButton({ action }: { action: () => Promise<void> }) { return <form action={action}><ArchiveClientSubmit /></form>; }
function ArchiveClientSubmit() { const { pending } = useFormStatus(); return <button aria-live="polite" className="client-archive-action" disabled={pending} onClick={(event) => { if (!window.confirm("Archive this client? Existing case history will remain available.")) event.preventDefault(); }} type="submit">{pending ? "Archiving…" : "Archive client"}</button>; }
