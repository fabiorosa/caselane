...
export default async function PortalRequestsPage({ params, searchParams }: { params: Promise<{ organizationSlug: string }>; searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  return <main className="portal-page"><PortalHeader identity={identity} /><div className="portal-content">
    ...
    <section className="portal-request-list" aria-label="Requests">{requests.length ? requests.map((item) => <article key={item.id}><div><span className="portal-case-number">CL-{item.sequence}</span><h2><Link href={`/portal/${organizationSlug}/requests/${item.id}`}>{item.title}</Link></h2><p>{label(item.status)}</p></div><div className="portal-next-action"><span>Next action</span><strong>{nextAction(item.status)}</strong></div><time dateTime={item.lastActivityAt.toISOString()}>{item.lastActivityAt.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</time></article>) : <div className="portal-empty"><h2>{query.search ? "No requests match your search" : query.state === "open" ? "No open requests" : "No completed requests"}</h2><p>{query.search ? "Try another title or request number." : "When work reaches this stage, it will appear here."}</p>{!query.search && query.state === "open" ? <Link href={`/portal/${organizationSlug}/requests/new`}>Send your first request</Link> : null}</div>}</section>
  </div></main>;
}
