...
export async function inviteTeamMemberAction(slug: string, _: TeamActionState, formData: FormData): Promise<TeamActionState> {
  ...
  const result = await inviteMember(createInvitationRepository(getDatabase()), createLocalInvitationEmailAdapter(process.env.NODE_ENV === "production"), systemClock, { organizationId: context.id, organizationName: context.name, userId: context.user.userId, role: context.role, appUrl: getServerEnvironment().APP_URL }, { email: String(formData.get("email") ?? ""), role: String(formData.get("role") ?? "") });
